...
def get_user_recent(request):
    ...
    if request.method == 'POST':
        context = {}
        context['recent_user'] = MyUser.objects.filter(is_active=1).order_by("-id")[:10]
...
def userList(request):
    ...
    context = {}
    sql = """
    select id,username,url,`desc`,avatar from user order by id desc;     
    """
    context['users'] = SelectAllSqlByColumns(sql, ['id', 'username', 'url', 'desc', 'avatar'])
